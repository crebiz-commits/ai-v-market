...
--     모바일(FeedAdCard=.discovery-section ~정사각/세로) 규격이 근본적으로 달라,
--     object-cover 로 한 이미지를 양쪽에 쓰면 한쪽이 잘림(16:9로 편집→모바일 좌우 잘림).
...
COMMENT ON COLUMN public.ads.image_url_mobile IS
  '모바일 피드 카드용 세로/정사각 광고 이미지(선택). NULL이면 image_url로 폴백. 2026-07-18.';
