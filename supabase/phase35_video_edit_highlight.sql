-- ════════════════════════════════════════════════════════════════════════════
-- Phase 35 — 영상 편집 모달에 "하이라이트 구간" 수정 추가 (2026-07-03)
--
-- 목적: 업로드 후에도 크리에이터가 본인 영상의 하이라이트 구간(홈 피드/OTT 히어로
--       미리보기용 시작~끝 초)을 수정 가능. 기존 update_my_video_metadata 에
--       하이라이트 파라미터가 없어 업로드 때만 설정 가능했음(가격 수정과 동일 누락).
--
-- 변경: p_highlight_start / p_highlight_end (REAL) 2개 추가. NULL=변경없음.
--       videos.highlight_start/highlight_end 컬럼은 REAL(초, 소수 허용).
--
-- 실행: Supabase Dashboard → SQL Editor → 본 파일 전체 복붙 → Run
-- ════════════════════════════════════════════════════════════════════════════

-- 기존 update_my_video_metadata 모든 시그니처 일괄 삭제 (파라미터 추가 = 새 오버로드
-- 발생 → 모호성 방지 위해 전부 DROP 후 재생성)
DO $$
DECLARE
  r RECORD;
BEGIN
...
COMMENT ON FUNCTION public.update_my_video_metadata IS
  '본인 영상의 메타데이터 일괄 갱신 (Phase 22 + 26 + 33 + 34[가격] + 35[하이라이트] 확장).';
