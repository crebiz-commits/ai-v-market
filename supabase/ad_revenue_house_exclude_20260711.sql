-- ════════════════════════════════════════════════════════════════════════════
-- F1 (2026-07-11): 크리에이터 광고수익 분배에서 하우스(자체)광고 노출 제외
--
--   문제: record_ad_impression 은 house 광고(ads.budget_krw IS NULL)에도
--         ad_video_events(수익 분배용) 를 기록하지만, 과금(spent_krw)은 예산광고만.
--         → calculate_monthly_revenue 의 광고 CTE 가 유료/하우스를 구분하지 않아
--           광고주가 ₩0 낸 하우스 노출까지 크리에이터에게 CPM×share 지급 = 플랫폼 순손실.
--
--   해결: 광고 관련 CTE(활동 UNION + ad_impressions)에 `JOIN ads ad ... AND
--         ad.budget_krw IS NOT NULL` 추가 → 유료광고 노출만 크리에이터 수익으로 집계.
--         크리에이터 광고수익 = 실제 수납 광고비 × share 로 정합.
--
--   ※ settlement_zero_correction_20260708.sql(정본: M1 실수납풀 + B-1 환불정정) 전체
--     재정의 + 위 2곳만 변경. 판매·구독·B-1·R7·UPSERT 로직은 100% 동일 유지.
--     ★ 이 파일이 calculate_monthly_revenue 새 정본. 07-08/07-03 등 구본 재실행 금지
--       (하우스 제외 필터 소실 → 손실 회귀).
--
...
BEGIN
  ...
  WITH activity AS (
    ...
    UNION
    -- 광고 노출 활동 (F1: 유료광고만 — house 광고 budget NULL 제외)
  ...
  ad_impressions AS (
    ...
    -- F1(2026-07-11): 유료광고(ad.budget_krw IS NOT NULL)만 집계 — house 광고 제외.
    SELECT
      v.creator_id AS cid,
...
COMMENT ON FUNCTION public.calculate_monthly_revenue IS
  '월별 정산 실행 (어드민 전용). 판매+광고+구독료 풀 분배 → revenue_distributions UPSERT. 구독 풀은 실제 수납 결제액(payments) 기준(M1). 광고수익은 유료광고 노출만 집계(house 제외, F1 2026-07-11). 기존 정산행 보유자는 활동 0이어도 재산출(환불 정정, B-1). 최소정산 미달분 deferred 이월, 누계 도달 시 일괄 pending 승격';

-- ════════════════════════════════════════════════════════════════════════════
-- 검증:
--   -- house 광고(budget NULL) 노출만 있는 크리에이터는 ad_revenue=0 이어야 함
--   SELECT applied_rates->>'ad_impression_basis' FROM public.revenue_distributions
--   ORDER BY updated_at DESC LIMIT 1;                     -- 기대: 'paid_only'
--   -- 재정산 후 하우스 노출 크리에이터의 ad_revenue 감소 확인
--   SELECT creator_id, ad_revenue FROM public.calculate_monthly_revenue(2026, 6);
-- ════════════════════════════════════════════════════════════════════════════
