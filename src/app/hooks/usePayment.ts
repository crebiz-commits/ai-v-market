...
async function startBillingAuth(customerKey: string, customerEmail?: string) {
  ...
  // ★ 카드사 심사용 "결제경로 파일" 캡처 전용 우회(2026-08-10): localStorage 'creaite_pay_capture'=1 이면
  //   클라 사전게이트를 건너뛰어 Toss **테스트 결제창까지** 도달시킨다(⑥ 캡처용). 서버 게이트(billing-auth-
  //   confirm)는 그대로라 실제 구독은 완료 안 됨(무상 프리미엄 abuse 불가). 캡처 후 플래그 지우면 원상복구.
  const _captureMode = typeof localStorage !== "undefined" && localStorage.getItem("creaite_pay_capture") === "1";
  if (!_captureMode) {
    try {
