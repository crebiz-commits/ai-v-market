...
// 개인정보처리방침만 별도 개정일 — 2026-08-20 분석도구(GA4·Vercel Web Analytics) 도입에 따른
//   위탁·국외이전·쿠키(행태정보) 조항 개정. 이용약관·청소년보호정책은 내용 변경이 없어 기존 날짜 유지.
const PRIVACY_LAST_MODIFIED = "2026-08-20";
...
export function PrivacyPage({ onBack, onNavigate }: StaticPageProps) {
  ...
  return (
    <PageShell title={text.title} subtitle={`${text.lastModifiedLabel}: ${PRIVACY_LAST_MODIFIED}`} onBack={onBack} onNavigate={onNavigate}>
      <div className="bg-[#121212] p-6 rounded-2xl border border-white/5 text-sm text-gray-300 leading-relaxed space-y-5">
        {text.sections.map((s, i) => (
          <Section key={i} title={s.title}>
            <span dangerouslySetInnerHTML={{ __html: s.body }} />
...
const PRIVACY_KO = {
  ...
  sections: [
    ...
    { title: "5. 처리 위탁", body: "Supabase Inc.(DB·인증·스토리지), Bunny.net(영상 CDN), Vercel Inc.(호스팅·방문 통계), Google LLC(웹 분석 — Google Analytics), 토스페이먼츠 또는 카카오페이(결제), Resend(이메일 발송) 등." },
    { title: "6. 개인정보 국외 이전", body: '회사는 서비스 제공을 위해 아래와 같이 개인정보를 국외로 이전(보관)합니다.<br />· <strong class="text-white">Supabase Inc.</strong>(미국) — 계정·콘텐츠·DB 보관, 회원 탈퇴 시까지<br />· <strong class="text-white">Bunny.net</strong>(Bunnyway d.o.o., 슬로베니아·EU) — 영상 스트리밍, 회원 탈퇴 시까지<br />· <strong class="text-white">Vercel Inc.</strong>(미국) — 웹 호스팅·접속 정보·방문 통계<br />· <strong class="text-white">Google LLC</strong>(미국) — 웹 분석(방문 일시·방문 경로·기기/브라우저 정보·대략적 지역), 구글 애널리틱스 보관 설정에 따름(최대 14개월)<br />· <strong class="text-white">Resend</strong>(미국) — 이메일 발송(이메일 주소)<br />이전 시점·방법: 위 서비스 이용 시점에 정보통신망을 통해 전송됩니다. 이용자는 개인정보 국외 이전을 거부할 수 있으나, 거부 시 서비스 이용이 제한될 수 있습니다.' },
    { title: "7. 이용자 권리 (열람·수정·삭제·다운로드)", body: '이용자는 개인정보보호법에 따라 다음 권리를 행사할 수 있습니다:<br /><strong class="text-white">· 열람·수정:</strong> 마이페이지 → 프로필 편집에서 직접 수정<br /><strong class="text-white">· 데이터 다운로드:</strong> 마이페이지 → 설정 → "내 데이터 다운로드"에서 본인 데이터 전체 JSON 다운로드<br /><strong class="text-white">· 계정 삭제:</strong> 마이페이지 → 설정 → 위험 영역에서 요청. 30일 유예 후 영구 삭제. 그 전까지 언제든 취소 가능<br />기타 문의는 <a href="mailto:support@creaite.net" class="text-[#8b5cf6] hover:underline">support@creaite.net</a>으로 연락주세요.' },
    { title: "8. 쿠키 및 행태정보 수집", body: '서비스는 로그인 유지 및 사용성 개선을 위해 쿠키를 사용합니다.<br />또한 서비스 이용 현황 분석(방문자 수·인기 콘텐츠·유입 경로 파악)을 위해 <strong class="text-white">Google Analytics</strong> 및 <strong class="text-white">Vercel Web Analytics</strong>를 이용하며, 이 과정에서 방문 일시·방문 페이지·유입 경로·기기 및 브라우저 정보·대략적 지역 정보가 수집됩니다. 수집된 정보는 통계 목적으로만 이용하며 이용자를 직접 식별하는 데 사용하지 않습니다.<br /><strong class="text-white">거부 방법:</strong> 브라우저 설정에서 쿠키를 차단하거나, 구글이 제공하는 <a href="https://tools.google.com/dlpage/gaoptout" target="_blank" rel="noopener" class="text-[#8b5cf6] hover:underline">Google Analytics 차단 브라우저 부가기능</a>을 설치하면 수집을 거부할 수 있습니다. 쿠키를 차단할 경우 로그인 유지 등 일부 기능이 제한될 수 있습니다.' },
    ...
    { title: "13. 개인정보 보호 책임자", body: '성명: 이현우 (크레비즈 대표)<br />이메일: <a href="mailto:support@creaite.net" class="text-[#8b5cf6] hover:underline">support@creaite.net</a><br />이용자는 서비스 이용 중 발생하는 개인정보 보호 관련 문의·불만·피해구제를 위 책임자에게 신고할 수 있으며, 회사는 지체 없이 답변·처리합니다.' },
...
const PRIVACY_EN = {
  ...
  sections: [
    ...
    { title: "5. Processors", body: "Supabase Inc. (database, authentication, storage), Bunny.net (video CDN), Vercel Inc. (hosting, visit analytics), Google LLC (web analytics — Google Analytics), Toss Payments or Kakao Pay (payment), Resend (email delivery), etc." },
    { title: "6. Cross-border Transfer of Personal Information", body: 'For service provision, personal information is transferred (stored) overseas as follows:<br />· <strong class="text-white">Supabase Inc.</strong> (USA) — account / content / database, until withdrawal<br />· <strong class="text-white">Bunny.net</strong> (Bunnyway d.o.o., Slovenia / EU) — video streaming, until withdrawal<br />· <strong class="text-white">Vercel Inc.</strong> (USA) — web hosting / access info / visit analytics<br />· <strong class="text-white">Google LLC</strong> (USA) — web analytics (visit time, referral path, device/browser info, approximate region); retained per Google Analytics retention settings (up to 14 months)<br />· <strong class="text-white">Resend</strong> (USA) — email delivery (email address)<br />Time / method of transfer: transmitted over the network at the time the above services are used. You may refuse the cross-border transfer, but service use may be restricted as a result.' },
    { title: "7. User Rights (Access, Edit, Delete, Download)", body: 'Users may exercise the following rights under personal information protection law:<br /><strong class="text-white">· Access / Edit:</strong> Edit directly via My Page → Edit Profile<br /><strong class="text-white">· Download:</strong> Download all your data as JSON via My Page → Settings → "Download my data"<br /><strong class="text-white">· Account deletion:</strong> Request via My Page → Settings → Danger Zone. Permanent deletion after a 30-day grace period; cancelable at any time before<br />For other inquiries, please contact <a href="mailto:support@creaite.net" class="text-[#8b5cf6] hover:underline">support@creaite.net</a>.' },
    { title: "8. Cookies and Behavioral Data", body: 'The Service uses cookies to maintain login sessions and improve usability.<br />The Service also uses <strong class="text-white">Google Analytics</strong> and <strong class="text-white">Vercel Web Analytics</strong> to analyze service usage (visitor counts, popular content, referral paths). This collects visit time, pages viewed, referral path, device and browser information, and approximate region. Collected data is used for statistical purposes only and is not used to directly identify individual users.<br /><strong class="text-white">How to opt out:</strong> You may block cookies in your browser settings, or install the <a href="https://tools.google.com/dlpage/gaoptout" target="_blank" rel="noopener" class="text-[#8b5cf6] hover:underline">Google Analytics Opt-out Browser Add-on</a>. Blocking cookies may limit some functions such as staying signed in.' },
    ...
    { title: "13. Personal Information Officer", body: 'Name: Lee Hyunwoo (이현우, CEO of Crebiz)<br />Email: <a href="mailto:support@creaite.net" class="text-[#8b5cf6] hover:underline">support@creaite.net</a><br />Users may report privacy-related inquiries, complaints, or remedy requests to the officer above, and the Company will respond without delay.' },
