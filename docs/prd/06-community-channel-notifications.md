...
> - DB/Edge: `supabase/features_tables.sql`, `community_upgrade_20260610.sql`, `community_security_20260621.sql`, `collab_space.sql`, `collab_inquiries.sql`, `collab_notify_privacy_20260614.sql`, `phase10_reports.sql`, `community_reports_hardening_20260707.sql`, `community_m1_posts_lockdown_20260707.sql`, `creator_followers.sql`, `channel_feed_audit_20260709.sql`, `phase34_notifications.sql`, `phase_notifications_realtime.sql`, `phase_web_push_20260531.sql`, `new_video_follower_notify_20260612.sql`, `medium_fixes_db_20260614.sql`, `notification_audit_20260710.sql`, `notification_audit2_20260710.sql`, `notification_audit3_20260710.sql`, `fix_moderation_rpc_collab_count_20260713.sql`, `functions/server/index.ts`(/send-email, /broadcast-email, /broadcast-push, /translate-post)
...
핵심 보안 원칙: 작성자명/아바타는 **서버 트리거로 강제**(위장 차단, 어드민의 'CREAITE 운영팀' 명의만 예외 — §6.1, `community_security_20260621.sql:11-27`), 타인에게 가는 이메일(actor 타입)은 **클라 콘텐츠를 신뢰하지 않고 서버 템플릿으로 고정**(`functions/server/index.ts:1748-1768`), 모든 알림 발송(벨 포함)은 **채널 3종('inapp'|'email'|'push') 사용자 설정 게이트**를 통과해야 한다(`should_send_notification`, 최신 정본 `notification_audit3_20260710.sql:47-82`).
...
- **게시글 피드** (`posts` 탭, `Community.tsx:910-`):
  ...
  - 외부 광고 슬롯은 **프리미엄 포함 전체 사용자**에게 노출(`ExternalAdSlot`, `Community.tsx:1028`; 과거의 비프리미엄 한정 게이트는 제거됨).
...
- 사이드패널(`w-80`). 헤더에 미읽음 배지 + "모두 읽음" — 배지는 목록 30개 상한과 무관한 **정확한 unread 총계**(`unreadTotal`, `:90,214`) 기준.
- 상태: `notifications/loading` + `unreadTotal`. 미인증 시 **i18n 단일 `SAMPLE`**(`useMemo`, `:62`; 과거 `SAMPLE_KO/EN` 이중 상수는 폐기). 샘플 읽음 상태는 **localStorage 영속**(`SAMPLE_READ_KEY`, `:19-26` — 재오픈 시 "읽었는데 또 안읽음" 방지).
- **패널 자체 realtime 구독**(`notif-panel-<userId>`, `:143`): 패널이 열려 있는 동안 새 알림 INSERT 가 목록에 즉시 반영(App 전역 구독과 별개).
- 타입별 아이콘/배경: `like/comment/purchase/sale/system/challenge/collab`.
- 클릭 → 읽음 처리 + `link` 로 네비게이션 + `unreadTotal` 감산(`:184`; 샘플 id `s` 접두는 이동 안 함).
...
### 4.1 게시글 CRUD
- **작성**: `handleWritePost`(`Community.tsx:755-807`). `community_posts` insert, payload `title/content/category/video_id/prompt_text/is_notice`(`:762-769`). `is_notice` 는 `profile.is_admin && writeNotice` 일 때만 true(`:768`). `author_name/author_avatar` 를 클라가 보내지만 → **서버 트리거가 프로필 값으로 덮어씀**(단, 어드민의 'CREAITE 운영팀' 명의는 예외 — §6.1·§12, `community_security_20260621.sql:11-27`).
- **수정**: 동일 함수, `editingPostId` 분기(`:770-782`). RLS `auth.uid()=user_id`.
- **삭제**: `handleDeletePost`(`:822-837`). RPC `delete_community_post`(댓글까지 cascade 삭제, **본인 OR `is_admin()`** 허용 — `fixes_audit_20260611.sql:352-357`) → 미적용 환경이면 `community_posts.delete` 폴백(`:824-828`).
...
- **지원(관심)**: `apply_to_collab(p_post_id,p_message)`(`collab_space.sql:100-159`). 'ok'/'already' 반환. 본인 글/마감/미인증은 예외. 성공 시 `applicants_count++` + 작성자에게 `notifications(type='collab')` insert(`:144-155`). *(현재 프론트 미배선 RPC — §11.)*
- **비공개 문의 스레드** (`collab_inquiries.sql`):
  - 시작: `collab_inquire(p_post_id)` get-or-create 스레드 — **최신 정본 `fix_moderation_rpc_collab_count_20260713.sql:28-47`**. **새 스레드 생성 시 `applicants_count`+1**(2026-07-13 수정 — 기존엔 아무도 안 올려 카드의 "문의 N건"이 영구 0인 죽은 카운터였음). **카운터 의미 = 서로 다른 문의자(스레드) 수.** 기존 글은 실제 스레드 수로 백필(`:50-58`). 호출 `CollabInquiryModal.startInquiry`(`:140-148`).
...
- INSERT 성공 시(중복 아님)에만 새 팔로워 알림 `sendNotification({type:'new_follower'})`(`:88-105`), link=팔로워의 채널(서버가 직접 구성).
- **서버 24h 디듀프**: `/send-email` 이 동일 팔로워發 new_follower 알림이 24시간 내에 이미 있으면 전체 스킵(언팔→재팔 반복 스팸 차단, `index.ts:1730-1746`). 기타 actor 타입(comment_reply 등)은 동일 수신자·동일 link **10초 디듀프**(`:1780-1793`).
- 자기 팔로우 차단: 클라(`:65`) + DB CHECK `no_self_follow`(`creator_followers.sql:21`).

### 4.8 새 영상 팬아웃 알림
- DB 트리거 `tg_notify_followers_new_video` — **최신 정본 `notification_audit2_20260710.sql`**. 옛 정의(`new_video_follower_notify_20260612.sql`, `medium_fixes_db_20260614.sql`, `notification_audit_20260710.sql`)는 전부 SUPERSEDED — 재실행 금지.
- **발동 시점 = 검수 통과**: `AFTER INSERT OR UPDATE OF is_hidden, visibility`(`notification_audit2_20260710.sql:55-58`). 업로드는 검수 대기(`is_hidden=true`)로 INSERT 되므로 INSERT 만으론 발송 0건이었음 → "숨김/비공개 → 공개"로 **새로 전환되는 시점에 1회** 발송. 이미 공개였던 상태의 UPDATE(제목 수정 등)는 **재발송 금지**(`:31-35`).
- **opt-out 게이트(벨)**: `notification_preferences.inapp_new_video_from_followed`(`COALESCE(...,true)` → **기본 ON**, `:49`). 자기 자신 제외(`:48`). *(과거의 email opt-in(기본 OFF) 게이트는 사실상 발송 0건이라 폐기.)*
- 이메일/푸시가 아니라 **벨 알림**으로만 보냄(Resend 비용 절감).
...
- 발송 경로는 두 가지(별도 `/send-push` 엔드포인트는 **없음**): ① 개별 알림 — `/send-email` 내부 `sendWebPushToUser`(`index.ts:12,1847-1852`)가 `should_send(...,'push')` **독립 게이트** 통과 시 service_role 로 `push_subscriptions` 조회 후 web-push 발송, ② 어드민 대량 — `/broadcast-push`(`index.ts:1936`).
...
### 4.11 알림 발송 (actor 흐름 — 벨/이메일/푸시 3채널)
- 클라 `sendNotification`(`sendNotification.ts:48-81`) → Edge `/send-email`(`functions/server/index.ts:1667-`).
- 서버: 호출자 토큰 검증(`:1671-1676`) → `to` 무시·user_id 로 수신자 조회(`:1678,1696-1702`) → 타입별 권한(self/admin/actor, `:1685-1694`) → actor 디듀프(new_follower 24h / 기타 10초, `:1730-1746,1780-1793`) → **벨: `should_send(...,'inapp')` 게이트 통과 후 INSERT**(fail-open — 게이트 오류·컬럼 없음이면 발송 유지, 명시 opt-out(false)만 스킵, `:1795-1809`) → **이메일: `should_send(...,'email')` 게이트** 후 Resend + `log_notification`(`:1811-1845`) → **푸시: 이메일과 분리된 `should_send(...,'push')` 독립 게이트** 후 `sendWebPushToUser`(`:1847-1852`).
...
| 테이블 | 정의 | RLS 요약 |
|---|---|---|
| `community_posts` | `features_tables.sql:98-111` (+`is_notice/video_id/prompt_text/is_hidden` 확장) | SELECT: 숨김 아님 OR 본인 OR admin(`community_security_20260621.sql:35-41`). INSERT/UPDATE: 본인 + 공지는 admin만(`community_upgrade_20260610.sql:30-41`). UPDATE 는 **컬럼단위 잠금** — 편집 9컬럼(title/content/category/video_id/prompt_text/is_notice/author_name/author_avatar/updated_at)만 GRANT, likes_count·comments_count·is_hidden·user_id 차단(`community_m1_posts_lockdown_20260707.sql:24-35`). DELETE: 본인(`features_tables.sql:128-129`; RPC `delete_community_post` 는 본인 OR admin) |
...
| `creator_followers` | `creator_followers.sql:16-22` | SELECT **본인 팔로잉만**(`auth.uid()=follower_id`, `channel_feed_audit_20260709.sql:208-212` — 팔로우 그래프 PII 비노출; 팔로워 수·팔로잉 여부는 DEFINER RPC 가 서빙), INSERT/DELETE 본인(`:34-44`), `no_self_follow` CHECK(`:21`) |
...
### 5.2 RPC 계약
- `create_report(text,text,text,text?) → bigint` — 신고 기록 + 자동숨김 + rate-limit(1시간 20건) + **대상 실존검증**(위조/미존재 target_id 로 인한 큐·자동숨김 카운트 오염 방지). SECURITY DEFINER. 최신 정본 `community_reports_hardening_20260707.sql`(원본 `community_security_20260621.sql:48-117`).
...
- `collab_inquire(uuid) → uuid` / `collab_thread_send(uuid,text) → (id,created_at)` / `collab_threads_for(uuid)` / `collab_thread_mark_read(uuid)` (`collab_inquiries.sql`; send 최신본 `collab_notify_privacy_20260614.sql`, inquire 최신본 `fix_moderation_rpc_collab_count_20260713.sql:28-47` — 새 스레드 생성 시 applicants_count+1).
- `get_my_following_videos(int)` / `get_popular_creators(int)` / `get_creator_profile(uuid)` / `get_creator_videos(uuid,int)` — 채널 RPC 4종의 **최신 정본은 `channel_feed_audit_20260709.sql`**(creator_of_month_until 반환 추가·정지자 필터·이메일 아이디 폴백 제거; 원본 `creator_followers.sql:50-280`, GRANT anon+authenticated).
...
- 알림: `get_my_notification_preferences()` / `update_my_notification_preferences(jsonb)` / `should_send_notification(uuid,text,text)→bool` / `log_notification(...)` (원본 `phase34_notifications.sql:87-244`). `should_send_notification` 의 채널은 **3종('inapp'|'email'|'push')** — inapp 은 opt-out 컬럼 미존재 시 fail-open(true), email/push 는 fail-closed. 최신 정본 `notification_audit3_20260710.sql:47-82`(update RPC 는 `:84-134`).
- 푸시: `save_push_subscription(text,text,text,text?)` / `delete_push_subscription(text)` (`phase_web_push_20260531.sql:31-67`).
- `delete_community_post(p_post_id)` — 글+댓글 cascade 삭제, **본인 OR `is_admin()`** 허용(`fixes_audit_20260611.sql:341-368`; 호출 `Community.tsx:824`).
...
- `trg_notify_followers_new_video` — 새 영상 팬아웃, **AFTER INSERT OR UPDATE OF is_hidden, visibility**(최신 정본 `notification_audit2_20260710.sql:19-58`).
- `orders_notify_seller` / `comments_notify_owner` — 판매·신규 댓글 벨 커버리지 트리거(`notification_audit_20260710.sql:48-123`; `inapp_sale`/`inapp_comment` opt-out 게이트 추가본 `notification_audit3_20260710.sql:137-207`).
...
1. **작성자명 서버 강제** — `community_posts`/`collab_posts` INSERT·UPDATE 시 트리거가 `profiles.display_name`(없으면 'CREAITE')·`avatar_url` 로 덮어씀. 클라 입력 무시. **예외: 어드민이 `author_name='CREAITE 운영팀'` 으로 보낸 경우만 공식 운영팀 명의 허용**(아바타는 공식 로고 고정; 비관리자는 어떤 값을 보내도 프로필로 강제 → 위장 차단 유지, `community_security_20260621.sql:11-27`).
...
6. **알림 채널 3종 게이트** — 모든 발송(벨 포함)은 `should_send_notification(user,type,channel)` 통과 필수. 채널은 **'inapp'|'email'|'push' 3종**(최신 정본 `notification_audit3_20260710.sql:47-82`). 벨(inapp)은 opt-out 컬럼 없는 타입이면 **fail-open**(발송 유지), 이메일/푸시는 fail-closed. **푸시 게이트는 이메일과 분리된 독립 판단**이며 `push_*` 는 **기본 true**(웹푸시 구독 자체가 동의 — 기존행 백필, `:28-44`). 벨 새 영상(`inapp_new_video_from_followed`)도 opt-out 방식 기본 ON(§4.8). 이메일 새 영상/답글은 여전히 기본 OFF.
...
10. **actor 이메일은 서버 템플릿 고정** — 타인에게 가는 **이메일은 클라 subject/html 무시**, 서버 고정 템플릿 사용(피싱 차단, `functions/server/index.ts:1748-1768`). 단 **인앱/푸시 `link` 는 same-origin 상대경로(`/`로 시작, `//`·`/\` 아님)면 클라 값 허용**(`:1772-1778` — `App.handleNotificationNavigate` 가 화이트리스트 파라미터만 추출하므로 외부 이동 불가). new_follower 의 body/link 는 서버가 직접 구성.
...
- **중복 팔로우**: insert 23505 무시(`useFollows.ts:84`), 알림도 미발송(`:88`). 서버 24h 디듀프(§4.7)가 언팔→재팔 반복 스팸을 추가 차단.
...
- **RLS 본인/공개/admin 매트릭스**: §5.1 참조. 공개 SELECT(community_posts·challenges·collab_posts), 본인 한정(post_bookmarks·notification_preferences·push_subscriptions·notification_log·**creator_followers** — 2026-07-09 팔로우 그래프 PII 비노출로 좁힘), 참여자 한정(collab_threads/messages), admin 게이트(challenges 관리·reports 조회·공지).
...
- **send-email actor 템플릿** — self/admin/actor 권한 분기(`index.ts:1685-1694`), actor 이메일은 서버 고정 subject/html(`:1748-1768`), 인앱/푸시 link 만 same-origin 상대경로 클라 값 허용(`:1772-1778`), 수신자 이메일은 항상 서버 조회(오픈릴레이 차단, `:1696-1702`).
...
- **발송 감사**: `notification_log`(channel/type/status/resend_message_id, `phase34_notifications.sql:58-70`) — 발송 성공/실패 추적. **설정 OFF 로 인한 skip 은 무기록**(설계 결정 2026-07-13 — 스킵 로깅은 노이즈·쓰기 비용 대비 무가치).
...
- [ ] 자기 자신은 팔로우할 수 없고(클라+DB), 중복 팔로우는 무시되며, 새 팔로워 알림은 INSERT 성공 시 1회 + 서버 24h 디듀프로 언팔→재팔 스팸이 차단된다.
- [ ] "구독" 피드가 팔로우한 채널의 공개 영상만 모아 보여준다.
- [ ] 앱을 켜둔 상태에서 새 알림 INSERT 시 벨 배지 +1 + 토스트가 뜬다(본인 알림만).
- [ ] 새 영상 팬아웃 벨은 **검수 통과(숨김/비공개 → 공개 전환) 시점에 1회** 발동하고, opt-out 하지 않은 팔로워(기본 ON)에게 자기 자신 제외하고 발송되며, 이미 공개였던 영상의 수정에는 재발송되지 않는다.
- [ ] 모든 채널(벨/이메일/푸시)은 사용자 설정(should_send) OFF면 해당 채널만 skip 된다. **skip 은 무기록**(설계 결정 2026-07-13 — 스킵 로깅은 노이즈·쓰기 비용 대비 무가치).
...
- **협업 지원 vs 문의 이원화** — `collab_applications`(관심 기록, `apply_to_collab`)와 `collab_threads`(비공개 대화) 두 모델이 공존. 현재 UI 는 문의 스레드만 사용(`CollabInquiryModal`; `apply_to_collab` 은 프론트 미배선). "문의 N건" 카운터는 `collab_inquire`(새 스레드 생성)가 담당 — §4.6, 2026-07-13 수정.
- **푸시 발송 경로** — 별도 `/send-push` 엔드포인트는 없음(과거 서술 폐기). 개별 알림은 `/send-email` 내부 `sendWebPushToUser`(push 독립 게이트), 대량은 `/broadcast-push`(§4.10). 운영 활성화는 VAPID private key 배포 의존(`webPush.ts:13-17`).
...
- **미인증 샘플 알림** — 로그인 전 벨에 i18n 샘플(`SAMPLE`) 노출(`NotificationPanel.tsx:62`); 실데이터와 혼동 방지 위해 id `s` 접두로 클릭 무효화, 읽음 상태는 localStorage 영속(`:19-26`).
...
## 12. 2026-07 전수 감사 반영 신규 기능

1. **운영팀 명의 게시 + 공지 자동 영문 번역** — 어드민 글쓰기 모달의 "운영팀 명의로 게시" 토글(기본 ON, `Community.tsx:420,1575-1586`)을 켜면 `author_name='CREAITE 운영팀'` + 공식 로고 아바타로 저장(트리거 예외 — §6.1). 운영팀 명의 저장/수정 시 Edge `/translate-post`(관리자 전용, Claude 번역)로 `title_en/content_en` 자동 생성·저장(`functions/server/index.ts:534-584`, 호출 `Community.tsx:769-791`). API 키 미설정(503)·실패 시 조용히 스킵 — 영문 모드는 한글 폴백.
2. **벨 타입별 opt-out 설정 UI** — 알림 설정에 벨(인앱) 섹션 `BELL_ITEMS` **9종**(새 영상·판매·새 댓글·답글·새 팔로워·정산·결제 영수증·신고 결과·환불, `NotificationSettings.tsx:54-64`) + **"이 기기" 웹푸시 통합 토글**(`subscribeToPush/unsubscribeFromPush`, `:134-160,235-`). `inapp_*` 저장은 `update_my_notification_preferences`(`notification_audit3_20260710.sql:84-134`).
3. **커버리지 트리거(판매/신규 댓글 벨)** — 구매 완료 시 판매자에게 `sale` 벨(`orders_notify_seller`), 영상/글에 새 댓글 시 콘텐츠 주인에게 `comment` 벨(답글·숨김·본인 제외)(`comments_notify_owner`, `notification_audit_20260710.sql:48-123`). `inapp_sale`/`inapp_comment` opt-out 게이트는 `notification_audit3_20260710.sql:137-207`.
4. **차단 사용자 글 숨김** — 차단한 사용자의 커뮤니티 글을 피드에서 제외(영상=DiscoveryFeed·댓글=CommentPanel 과 동일 약속). **공지(isNotice)는 예외**(운영팀 글 — 항상 노출)(`Community.tsx:367,899-908`).
...
│  │ ♥ 12   💬 4   🔖   ⚐ 신고                                 │  │
│  └─────────────────────────────────────────────────────────┘  │
│  ┌─ [외부 광고 슬롯] ExternalAdSlot (전체 노출 :1028) ───────┐  │
│  └─────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────┘
```
...
│ │ [ ] is_admin 일 때만 의미                │          │
│ └──────────────────────────────────────┘          │
│ [✓] 운영팀 명의로 게시 (어드민 전용, 기본 ON)         │  ← writeAsOfficial :420, §12
│     → 'CREAITE 운영팀' + 공식 로고, 저장 후 자동 영문번역 │
│                          [취소]   [등록하기]        │
...
│ 💰 정산이 완료되었습니다                     ← sale         │
│   (타입별 아이콘/배경 맵)                                  │
│   클릭 → 읽음 처리 + link 이동 + unreadTotal 감산 (:184)   │
│   ※ 배지 = 정확한 unread 총계(30개 상한 무관, :90,214)      │
│   ※ 패널 자체 realtime 구독(notif-panel-<uid>, :143)       │
│   ※ 미인증이면 i18n SAMPLE(읽음 localStorage 영속),         │
│      id 's' 접두는 이동 안 함                               │
└───────────────────────────────────────────────────────────┘
```
...
sequenceDiagram
    ...
    alt INSERT 성공(중복 아님)
        ...
        Note over SN: actor 타입 → 서버 템플릿 + 24h 디듀프 (index.ts:1730-1746)
    else 23505 (이미 팔로우)
        H->>H: 무시, 이메일 미발송 (:84)
    end

    Note over DB: --- 별개 이벤트: 팔로우 대상의 영상이 검수 통과(공개 전환) ---
    participant V as videos INSERT/UPDATE
    participant T as tg_notify_followers_new_video
    V->>T: 공개 전환 — INSERT 즉시공개 또는 is_hidden/visibility UPDATE<br/>(notification_audit2_20260710.sql:55-58)
    T->>T: 이미 공개였던 UPDATE 는 재발송 금지 (:31-35)
    T->>DB: 팔로워 조회 (자기 제외 :48,<br/>inapp_new_video_from_followed=true 기본 ON :49)
...
|---|---|---|---|---|
| `community_posts` | 숨김아님 OR 본인 OR admin | 본인(공지는 admin) | 본인 + UPDATE 는 편집 9컬럼만 GRANT(컬럼단위 잠금) | `community_security_20260621.sql:35-41`, `community_upgrade_20260610.sql:30-41`, `community_m1_posts_lockdown_20260707.sql:24-35`, `features_tables.sql:128-129` |
...
| `creator_followers` | 본인 팔로잉만 | 본인(`no_self_follow`) | DELETE 본인 | `channel_feed_audit_20260709.sql:208-212`, `creator_followers.sql:16-22,34-44` |
...
|---|---|---|---|---|
| `create_report` | `(p_target_type text, p_target_id text, p_reason text, p_description text=null)` | `bigint` (report_id) | DEFINER, authenticated. auth 필수, 1h 20건 제한, 본인 user 신고 차단, 자동숨김, 대상 실존검증 | 최신 정본 `community_reports_hardening_20260707.sql`(원본 `community_security_20260621.sql:48-117`) |
...
| `collab_inquire` | `(p_post_id uuid)` | `uuid` (thread_id, get-or-create) | DEFINER. 본인글/마감 예외. 새 스레드 생성 시 `applicants_count`+1(=서로 다른 문의자 수) | 최신 정본 `fix_moderation_rpc_collab_count_20260713.sql:28-47`(원본 `collab_inquiries.sql:62-76`) |
...
| `should_send_notification` | `(p_user_id uuid, p_type text, p_channel 'inapp'|'email'|'push')` | `bool` | DEFINER, authenticated+service_role. inapp 은 opt-out 컬럼 미존재 시 fail-open(true), email/push 는 fail-closed | 최신 정본 `notification_audit3_20260710.sql:47-82`(원본 `phase34_notifications.sql:168-209`) |
...
| `get_my_following_videos` / `get_popular_creators` | `(int limit)` | `TABLE` | anon+authenticated GRANT. 채널 RPC 4종 최신 정본 = `channel_feed_audit_20260709.sql` | `channel_feed_audit_20260709.sql`(원본 `creator_followers.sql:50-280`) |

### Edge Functions (`functions/server/index.ts`)

| 엔드포인트 | 인자(body) | 반환 | 권한/동작 | file:line |
|---|---|---|---|---|
| `POST /send-email` | `{user_id, type, subject, html, link?}` (`to`는 무시) | `{success, skipped?, reason?, message_id?}` | Bearer 토큰 검증. self/admin/actor 권한 분기. actor=이메일 서버 고정 템플릿(인앱/푸시 link 는 same-origin 상대경로만 클라 값 허용). 수신자=user_id로 서버 조회. actor 디듀프(new_follower 24h/기타 10초) → should_send(inapp) 게이트 후 벨 INSERT → should_send(email) 게이트 후 Resend+log_notification → should_send(push) 독립 게이트 후 sendWebPushToUser | `index.ts:1667-1859` |
| `POST /broadcast-email` | (어드민 대량 발송 페이로드) | `{...}` | 어드민 대상 일괄 이메일 | `index.ts:1867-` |
| `POST /broadcast-push` | (어드민 대량 발송 페이로드) | `{...}` | 어드민 대상 일괄 웹푸시 | `index.ts:1936-` |
| `POST /translate-post` | `{title, content}` | `{ok, title_en, content_en}` | 어드민 전용. 운영팀 공지/게시글 영문 자동 번역(Claude). API 키 미설정 시 503 → 호출측 스킵(한글 폴백) | `index.ts:534-584` |
...
기능: 팔로우·알림
  ...
  시나리오: 중복 팔로우 (엣지)
    ...
    그러면 23505 가 무시되고 새 팔로워 알림이 중복 발송되지 않는다

  시나리오: 새 팔로워 알림 1회 + 24h 디듀프
    조건 처음으로 팔로우(INSERT 성공)한다
    그러면 new_follower 알림이 정확히 1회 발송된다
    조건 언팔 후 24시간 내에 다시 팔로우한다
    그러면 서버가 동일 팔로워發 알림을 스킵한다(24h 디듀프 — 스팸 차단)
  ...
  시나리오: 새 영상 팬아웃 벨 — 검수 통과 시 1회 (엣지)
    조건 업로드가 검수 대기(is_hidden=true)로 INSERT 된다
    그러면 벨이 발송되지 않는다
    만약 검수 통과로 is_hidden 이 false 로 전환되면
    그러면 opt-out 하지 않은 팔로워(inapp_new_video_from_followed 기본 ON)에게 벨이 가고 자기 자신은 제외된다
    그리고 이미 공개였던 영상의 제목 수정 UPDATE 에는 재발송되지 않는다

  시나리오: 알림 설정 OFF skip (채널 독립)
    조건 사용자가 해당 종류의 이메일을 OFF 했다
    만약 /send-email 이 호출되면
    그러면 should_send_notification(email) 이 false → 이메일만 skip 된다
    그리고 벨(inapp)·푸시(push) 게이트는 각각 독립적으로 판단된다
    그리고 skip 은 notification_log 에 기록하지 않는다(무기록 설계 — 2026-07-13)
...
- 모든 채널(벨/이메일/푸시)은 각자의 should_send 게이트를 독립 통과해야 하며, 이메일 발송 성공/실패는 notification_log 에 기록된다(설정 OFF skip 은 무기록 — 2026-07-13 설계 결정).
- 타인에게 가는 이메일 본문은 서버 고정 템플릿이며 사적 메시지 원문을 노출하지 않는다(인앱/푸시 link 만 same-origin 상대경로 클라 값 허용).
...
| 날짜 | 내용 |
|---|---|
| 2026-07-13 | 전수 감사 반영 — 알림 3채널 게이트·팔로우 RLS·문의 카운터 등 |
