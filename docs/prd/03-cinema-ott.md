...
- (관람객) OTT 탭에 들어가면 트렌딩 상위 작품이 풀블리드로 자동재생(음소거)되고, 20초마다 다음 작품으로 순환한다(`Ott.tsx:204-213`).
...
### 4.3 히어로 20초 순환 (OTT, `Ott.tsx:204-242`)
- `heroes` = 트렌딩 상위 5편(비면 장르 행 영상 폴백)(`Ott.tsx:204-206`).
- `heroIdx` 가 `setInterval(...,20000)` 마다 `(i+1)%length` 순환(`Ott.tsx:209-213`). 1편 이하면 순환 안 함.
- 히어로 영상 소스(`video_url` 등)는 RPC에 없어 `videos` 테이블에서 별도 조회(`Ott.tsx:215-242`). `heroSrcCache` 로 회전마다 재조회 방지(`Ott.tsx:150,219`).
- 재생 우선순위: 미리 잘린 `hero_clip_url`(30초 클립) 있으면 자동재생, 없으면 Bunny `preview.webp` 애니메이션, 그것도 없으면 썸네일 포스터(`Ott.tsx:440-443,233-236`).
- 화면 밖이면 IntersectionObserver 로 히어로 영상 일시정지(`Ott.tsx:453-463`).
...
- 장르 행 정렬: `bandRank()` 로 `order` 우선, "기타"(default)는 항상 맨 뒤(999), 나머지 알려진 장르=100(`Ott.tsx:134-139,197-202`).
...
- [ ] OTT 히어로: 트렌딩 상위 5편 20초 순환, 클립 있으면 자동재생·없으면 preview/썸네일(`Ott.tsx:204-242,440-443`).
