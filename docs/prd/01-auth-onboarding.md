...
> - `supabase/profiles_table.sql`, `supabase/referral_20260618.sql`, `supabase/referral_claim_atomic_20260707.sql`, `supabase/subscription_consistency_20260705.sql`, `supabase/block_suspended_writes_20260625.sql`, `supabase/fix_profiles_write_lockdown_20260628.sql`, `supabase/phase27_user_data_rights.sql`, `supabase/phase26_age_rating.sql`, `supabase/phase_security_hardening_20260531.sql`, `supabase/fix_profiles_column_exposure_20260625.sql`

## 개정 이력

| 날짜 | 내용 |
|---|---|
| 2026-07-13 | 전수 감사 반영 — 구독판정 P9·소셜정리·레퍼럴 원자화 등 |
...
2. **소셜 로그인**: Google·Kakao OAuth 지원. 로그인 모달의 진입 수단은 **이메일·카카오·구글 3개뿐** — Facebook/Apple/X/LINE 장식 버튼 4종은 제거됨(2026-07-10).
...
- **레이아웃**: 모바일은 바텀시트(스프링 슬라이드 업, `:111-117`), 데스크톱은 센터 카드(max 420px). 헤더(타이틀·닫기·뒤로가기), 본문, 하단 모드 전환 영역.
- **상태/분기**:
  - **소셜 목록**(`showEmailForm=false`, `:189`): **이메일(`:198`)·카카오(`:207`)·구글(`:220`) 3개 버튼뿐이며 전부 실연결.** Facebook/Apple/X/LINE 장식 버튼 4종(핸들러 없는 UI)은 2026-07-10 제거됨.
  - **이메일 폼**(`showEmailForm=true`, `:235`): signup이면 이름 필드 추가(`:243`), 이메일·비번(min 6자, `:281`). signin이면 "비밀번호 찾기" 링크 노출(`:285`).
  - **인증 메일 발송 안내**(`verifySentTo` 설정, `:146`): 받은 메일 안내 + "인증 메일 재발송"(`:171`) + "인증을 마쳤어요 — 로그인"(`:180`, signin 폼으로 복귀).
  - **로딩**: 제출 버튼 스피너+`disabled` 잠금(`loading`, `:297-303`). **카카오·구글 버튼에는 `disabled` 잠금 없음** — 클릭 즉시 OAuth 리다이렉트로 페이지를 떠나므로 무해(핸들러 `:81-101`).
  - **에러**: `toast.error`로 표시(`:75`,`:88`,`:98`). WebView 에러는 toast duration 8s(`:88`).
  - **성공**: signin/즉시가입은 `onClose()`(`:62`,`:71`). 인증 필요 가입은 안내 화면 전환(닫지 않음).
- **전환**: signin↔signup 토글(`:326`,`:336`)은 `showEmailForm=false`로 리셋해 소셜 목록부터 다시.
...
3. `claim_referral`(정본 `referral_claim_atomic_20260707.sql:13-47`, 최초 `referral_20260618.sql:116`): 가드 — 로그인됨/코드 유효/이미 연결 아님(`referred_by IS NULL`)/자기 자신 아님 → `referred_by` 기록 후, 연결 UPDATE의 `ROW_COUNT`가 1일 때만(**실연결 시에만**) 초대자 `referral_count+1`(C3 카운트 부풀리기 차단). 멱등.
...
| `claim_referral(p_code TEXT)` | 초대코드 | `BOOLEAN`(멱등, `ROW_COUNT` 가드 — 실연결 시에만 count+1) | SECURITY DEFINER | REVOKE PUBLIC → `authenticated` | 정본 `referral_claim_atomic_20260707.sql:13-50`(최초 `referral_20260618.sql:116`) |
...
| `is_subscriber(p_user_id UUID=auth.uid())` | 사용자 | `BOOLEAN`(P9: 만료일 NULL=비구독) | STABLE | — | 정본 `subscription_consistency_20260705.sql:17-26`(`profiles_table.sql:164-172`의 NULL=활성 정의는 폐기) |
...
4. **구독 활성 판정(P9, 2026-07-05)**: tier ≠ free AND **`subscription_expires_at IS NOT NULL`** AND 만료 미래 → `isSubscriber`, premium은 추가로 tier==='premium'(`AuthContext.tsx:422-427`,`:450-451`). **만료일 NULL = 비구독**으로 통일(서버 재생게이트와 일치) — 과거 "만료 없음 OR 만료 미래" 판정은 폐기. 영구 무료제공(관리자 컴프)은 NULL 대신 먼 미래 날짜(예: 2099-12-31)로 명시. DB `is_subscriber()`도 동일 판정(`subscription_consistency_20260705.sql:17-26`).
...
9. **정지자(`is_suspended`)**: 어드민 화면에서 표시·관리(`AdminUsers.tsx:148`, `AdminOverview.tsx:308`). `is_suspended`는 공개 노출 금지 민감 컬럼(`fix_profiles_column_exposure_20260625.sql:8,26`). 정지자의 사용자 쓰기(댓글·게시글·팔로우·좋아요·신고·업로드)는 DB 트리거 + Edge에서 **전면 차단**(§8-1, `block_suspended_writes_20260625.sql`).
...
- **signIn 응답에 토큰 누락**: 응답에 세션 토큰이 **없을 때만** fallback으로 수동 user/profile 설정(`:298-304`, 방어적). `setSession` 자체가 throw하면 fallback 없이 에러가 그대로 전파된다.
...
| authenticated(회원) | 본인 프로필 RPC(`get_my_profile`/`get_my_payout_info`), `claim_referral`/`get_my_referral`, 연령인증, 데이터 내보내기/삭제요청/취소, 본인 프로필 UPDATE(**편집 5컬럼만** — §8-2) |
...
### 8-1. 정지자(`is_suspended`) 쓰기 전면 차단 — `block_suspended_writes_20260625.sql`

- `is_self_suspended()`(SECURITY DEFINER, `auth.uid()` 기준 본인 정지 여부 조회, `:16-21`) + `tg_block_suspended()`(`:23-34`)가 정지자의 쓰기를 DB 레이어에서 예외(`check_violation`)로 차단 — 클라이언트/RPC 경유 무관.
- **BEFORE 트리거 부착 8개 테이블**(`:37-69`): `comments`·`community_posts`·`collab_posts`(INSERT OR UPDATE), `creator_followers`·`post_likes`·`comment_likes`·`video_likes`·`reports`(INSERT).
- **영상 업로드**는 Edge `save-metadata`(service_role)라 트리거 미적용 → `create-upload` Edge에서 별도 **403** 차단.
- service_role(시스템/Edge) 쓰기는 `auth.uid()=NULL`이라 차단되지 않음(정상 동작 보존).

### 8-2. profiles 쓰기 잠금(write lockdown) — `fix_profiles_write_lockdown_20260628.sql`

- `profiles`의 INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES를 anon/authenticated/PUBLIC에서 **전면 REVOKE**(`:20`) 후, 클라 직접 쓰기 유일 경로(MyPage 프로필 편집 upsert)의 **편집 5컬럼만 재부여**: UPDATE는 `display_name`·`bio`·`avatar_url`·`banner_url`·`updated_at`, INSERT는 upsert용 `id` 포함 6컬럼(`:24-25`).
- 그 외 모든 프로필 변경(정산계좌/세금/구독/정지/삭제/레퍼럴)은 SECURITY DEFINER RPC 경유라 테이블 GRANT 영향 없음 → 정상 동작 유지.
- 효과: `is_suspended` 자가 해제, `deletion_requested_at` 조작, `email`/`birthdate`/`business_*`/`tax_*` 직접 수정이 **컬럼 GRANT 부재로 권한 에러** 거부. `protect_subscription_columns` 트리거(보호 8컬럼 OLD 강제)는 이제 **심층방어(2차 방어선)**.
...
- `profiles` 쓰기(INSERT/UPDATE)를 테이블 단위로 재부여 금지 — 편집 5컬럼(+INSERT `id`)만 컬럼 단위 GRANT(§8-2, MEMORY: profiles-column-grant-ssot).
...
- [ ] 일반 사용자가 PostgREST로 `subscription_tier`/`is_admin`/`payout_info`/`referral_*` 직접 UPDATE 시 **컬럼 GRANT 부재로 권한 에러** 거부(§8-2 쓰기 잠금; 보호 트리거의 OLD 강제는 심층방어).
...
1. **(정리 완료 2026-07-10) 소셜 장식 버튼 4종 제거**: Facebook/Apple/X/LINE 버튼(핸들러 없는 장식 UI)은 제거됨 — 현재 로그인 모달은 이메일·카카오·구글 3개만. Apple 등은 iOS 출시 후 실연동으로 추가 예정.
...
모바일은 바텀시트(슬라이드 업), 데스크톱은 센터 카드(max 420px). (`AuthModal.tsx:111-117`,`:189`)
...
│                                     │
│   ┌───────────────────────────────┐ │
│   │  ✉  이메일로 계속하기          │ │ :198 → showEmailForm=true
│   └───────────────────────────────┘ │
│   ┌───────────────────────────────┐ │
│   │  💬  카카오로 계속하기   (연결) │ │ :207  signInWithKakao
│   └───────────────────────────────┘ │
│   ┌───────────────────────────────┐ │
│   │  G  Google로 계속하기    (연결) │ │ :220  signInWithGoogle
│   └───────────────────────────────┘ │
│   (FB/Apple/X/LINE 장식 버튼 4종은   │
│    2026-07-10 제거 — 위 3개가 전부)  │
│                                     │
├─────────────────────────────────────┤
│  계정이 없으신가요?  회원가입 ▸      │  ← 모드 토글 :326,:336
└─────────────────────────────────────┘
   (제출 버튼만 로딩 잠금 :297-303 — 카카오·구글은 disabled 없음, OAuth 리다이렉트라 무해)
```
...
| `claim_referral(p_code TEXT)` | 초대코드 | `BOOLEAN` (멱등, `ROW_COUNT` 가드 — 실연결 시에만 count+1) | REVOKE PUBLIC → `authenticated`, DEFINER | 정본 `referral_claim_atomic_20260707.sql:13-50`(최초 `referral_20260618.sql:116`) |
...
Feature: 보호컬럼·민감컬럼 차단
  ...
  Scenario: 엣지 — 일반 사용자의 보호컬럼 직접 UPDATE
    ...
    Then 컬럼 GRANT 부재(§8-2 쓰기 잠금)로 권한 에러가 발생해 거부된다
    And protect_subscription_columns 트리거는 심층방어로 OLD값 강제를 유지한다
    # [AC: 보호컬럼 직접 UPDATE 권한 에러 거부]
