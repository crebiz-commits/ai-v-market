...
sequenceDiagram
    ...
    alt 유예 내 취소
        ...
        Admin->>DB: purge_pending_deletions(30)  // phase27:92-126 (어드민 검사)
        DB->>DB: profiles DELETE → FK CASCADE
        Note over DB: auth.users는 별도 admin API 삭제 필요 (phase27:120-122)
    end
```
...
Feature: 데이터 내보내기 및 계정 삭제
  ...
  Scenario: 엣지 — 30일 경과 후 purge
    Given 삭제 요청 후 30일이 지난 행이 있다
    When 어드민/Cron이 purge_pending_deletions(30)을 호출한다
    Then 해당 profiles가 DELETE되고 FK CASCADE로 연관 데이터가 정리된다
    And auth.users는 별도 admin API로 삭제해야 한다
    # [AC: 30일+ 경과 시 purge로 영구 삭제]
```
