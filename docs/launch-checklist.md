...
- [ ] 🔴 **광고 노출/클릭 사기 방어 강화(#2) — 결제(토스) 라이브 전 필수.** dedup 키가 클라 생성이라 키 회전으로 우회 가능 → Edge 집계+신뢰IP+IP별 레이트리밋+raw RPC REVOKE. 설계: [ad-fraud-hardening-plan.md](ad-fraud-hardening-plan.md). (지금 자체광고 OFF·과금 전이라 실손해 0, 순진한 IP스왑은 CGNAT 과소집계라 금지.)
