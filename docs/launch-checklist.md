...
- [ ] **토스페이먼츠 가맹 계약·심사** → `live_` 키 발급 (선행: 사업자등록증 ✅ + 정산 계좌) — ⏳ **신청·결제 완료(2026-06-15), 심사 대기 중**(예상 1~2개월 → 2026-08-15경까지 무응답 시 진행상황 문의). 승인 나야 체크(= live 키 발급).
...
- [ ] ⏰ **2026-08-07 12:00 이후 → 프로덕션 재신청**. 1차 반려(2026-07-20, 실행 부족·1회차) 후 재도전. ⚠️ **2026-08-03 에 신청 버튼이 켜져도 누르지 말 것** — 1차 실패가 정확히 "버튼 켜져서 눌렀다 반려"였다. 그때까지 2일마다 Play Console `통계` 확인하고, 대시보드의 **"N일 동안 참여를 선택한 테스터 12명"** 이 매일 1씩 오르는지 볼 것. 남은 실질 작업은 **테스터에게 "하루 한 번 앱 열어달라" 연락**(반려 사유가 참여도). 상세: [WORK-HANDOFF.md](WORK-HANDOFF.md) ①
- [ ] ⏰ **2026-08-31 까지 → 타겟 API 36(Android 16) 빌드로 교체**. 현재 AAB 는 타겟 SDK 35(PWABuilder 기본값)라 기한 후 **업로드 거부**된다. ▣ 8/7 재신청에는 영향 없고(기한 전), 기존 앱이 스토어에서 사라지지도 않는다(업로드만 막힘). 대응: PWABuilder 재빌드 1순위 → 그때도 35 만 지원하면 Bubblewrap CLI 직접 빌드 또는 **연장 신청(2026-11-01 까지)**. 재빌드 시 [기본값 함정 3개](WORK-HANDOFF.md) 주의(Package ID·서명키·버전코드).
