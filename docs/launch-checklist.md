...
- [ ] 🟡 **시청 보호 잔여(2026-07-11 구매/시청 감사)** — Embed 토큰인증은 ON이라 임베드 플레이어는 프리미엄·연령 보호됨. **①연령게이트 — ✅ 발효 완료**(play-token 엔드포인트가 19금 미인증/비소유자에게 토큰 미발급. 코드 b34ce42(2026-07-11) → Edge 재배포 a50a679(2026-07-18)로 반영 확인. 2026-07-22 재확인 시 "재배포 필요"로 남아 있던 표기를 정정). **②잔여: CDN 토큰인증 OFF**라 `video_url`(anon 노출)·직접 `{id}/playlist.m3u8`·`play_720p.mp4` URL은 토큰 없이 접근 가능 → 비구독자/미성년이 직링크로 본편 시청 가능. CDN token auth ON 시 완전차단되나 preview.webp·썸네일 무토큰 서빙과 트레이드오프 → **출시 전 결정**(19금 실콘텐츠 생기기 전). 관련: [[upload-moderation-pipeline]] 잔여. **★2026-07-22 구매 탭 감사 보강 — 이건 시청 보호만의 문제가 아니다: 다운로드 판매(라이선스)도 같은 무토큰 CDN URL(`{id}/play_720p.mp4`)을 그대로 열어주므로, 구매하지 않은 사람도 videoId 만 알면 원본 파일을 받을 수 있다 → 라이선스 구매 자체가 무의미해진다.** 서버 게이트(`log_download`)는 정상 동작하지만 파일 자체가 공개라 우회된다. 유료 판매 개통 전 반드시 결정할 것(우선순위가 "19금 콘텐츠 생기기 전"보다 앞설 수 있음).
