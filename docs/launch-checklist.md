...
> 갱신: **2026-08-20** (이전 2026-06-14) · **완료 항목은 제거**하고 미해결만 추적. 완료 내역 요약은 맨 아래 "✅ 완료 기록".
>
> **★ 2026-08-20 현황 요약**: 🟢 **Google Play 정식 출시(LIVE)** · 🟢 카카오 애드핏 광고수익 발생 중 · 🟢 스토어 한글검색 최적화 제출 · 🔴 **유일 실질 블로커 = 토스 결제 승인**(심사 중, 독촉 발송) · 🟡 타겟API36(~8/28 재확인)·개발자인증(9/30 확인).
...
- [ ] **토스페이먼츠 가맹 계약·심사** → `live_` 키 발급 — ⏳ **심사 진행 중.** 담당 박제영(jeyoung.park@tossgrowth.com / 1544-7772). MID `creaitu20o`. **★2026-08-14 회신: MID 2개 필요 판명** — 현 `creaitu20o`=단건용, **구독(정기결제)은 빌링 MID 별도 신청** 필요. 단건은 이미 연동됐으나 게이트가 리뷰어를 막아 '미연동'으로 오인 → **테스트 계정만 게이트 우회**(`start_payment_toss_review_bypass_20260814.sql`, **심사 후 `start_payment_effective_setting_20260723.sql` 재실행으로 원복 필수**) 조치. **2026-08-20 독촉메일 발송**(진행상황+빌링MID 링크 요청). 승인 나야 체크. 관련 메모리: toss-payment-onboarding.
- [ ] **정기결제(빌링) MID 추가 신청** — "구독 정기결제" 선택해 별도 신청(신청 링크는 박제영에게 요청). 구독 실결제의 선행조건.
...
- [~] 시드 콘텐츠 — **자체 제작 AI 영상 21편 적재(서비스 중, 2026-07-21 기준)**. ※ 초기 외부 링크 시드(180+편)는 **전량 삭제** — 저작권·품질 리스크 제거. 편수는 줄었으나 전부 자체 원본이라 대외 설명이 오히려 명확해짐. ⚠️ **외부 크리에이터의 자발적 콘텐츠는 토스(정산)·구독자·트래픽이 생긴 뒤 따라오는 결과물**(선행 강요 불가 — 돈/관객 없는 곳에 안 올림). 운영자 마중물(본인 AI 원본)만 베타 직전 보강.
- [ ] 베타 테스터 모집 + 피드백 채널 운영
...
### ⏰ 진행 중 마감일 (2026-07-22 기준 — 서로 별개 건)
- [x] ✅ ⏰ **2026-08-07 프로덕션 재신청 → 완료·정식 출시(LIVE).** 재신청 제출(2026-08-10) → 프로덕션 액세스 승인 → **약 2026-08-16 정식 게시**(프로덕션 100%·대한민국·설치 18). 스토어 `https://play.google.com/store/apps/details?id=net.creaite.app`. 1차 반려(2026-07-20 참여도)는 재테스트로 해소. ⚠️ 이 폰 계정은 테스터라 스토어에 "(베타)"로 보임 — 일반 사용자는 정상 표시(문제 아님).
- [ ] ⏰ **2026-08-31 까지 → 타겟 API 36(Android 16) 빌드로 교체**. 현재 라이브 AAB 는 타겟 SDK 35. **라이브 유지엔 영향 없음**(target 35 는 기존앱 기준 충족 → 앱 안 내려감, 8/31 후 "새 업로드"만 막힘). **★2026-08-20 확인: PWABuilder 가 아직 타겟 36 미지원**(GitHub 이슈 #6160 미처리) → 지금 재빌드해도 35 만 나옴(헛수고). **계획: ~8/28 PWABuilder 36 지원 재확인 → 되면 재빌드·업로드, 안 되면 연장신청(11/1) 또는 Bubblewrap CLI.** 서명키 `D:\크리에잇 구글 스토어 PWABuilder\signing.keystore` 확인됨. 재빌드 시 [함정 3개](WORK-HANDOFF.md)(Package ID `net.creaite.app`·기존 서명키 재사용·버전코드 3+).
- [ ] ⏰ **2026-09-30 → Android 개발자 인증 + 앱 등록 확인**(2026-08-08 구글 메일). ★위 8/31 타겟API 와 **별개 제도** — 악성앱 방지용 "앱↔인증된 개발자 신원 연결". **크리에잇 실질 부담 거의 없음**: ①Play 앱 등록은 메일에도 "**99%+ 자동 등록**"이라 크리에잇 TWA 도 이미 자동 등록됐을 것 ②Play 외부 배포·추가 서명키 등록은 **해당 없음**(Play 전용, 사이드로드 아님) ③개발자 신원은 Play Console 계정($25·사업자정보) 만들 때 이미 확인됨. ▣ **차단 시행은 초기 4개국(브라질·인니·싱가포르·태국)뿐, 전세계는 2027, 한국은 초기 대상 아님** → 당장 급하지 않음. ▣ **할 일 = 딱 하나**: Play Console → "Android 개발자 인증" 페이지에서 크리에잇 앱이 **"등록됨"으로 뜨는지 확인**(별도 Android Developer Console 계정 불필요). 등록됨=끝, 미등록이면 그 페이지서 클릭 몇 번. 출처: [Android Developers 블로그](https://android-developers.googleblog.com/2026/03/android-developer-verification-rolling-out-to-all-developers.html)·[The Hacker News](https://thehackernews.com/2026/06/google-sets-sept-30-deadline-for.html).

### 앱 패키징 — 📘 [twa-build-guide.md](twa-build-guide.md) (절차 전체)
> 토스 무관 · 자체 제작 콘텐츠 21편 보유라 "빈 앱" 아님 → **무료 광고형으로 토스 없이 선출시 가능**(프리미엄 구독만 토스 후행).
...
- [x] ✅ ⭐ Play 앱서명 SHA-256 → `assetlinks.json` 기입·배포(URL바 제거 핵심) — 2026-07-21 확인. live `www.creaite.net/.well-known/assetlinks.json` 에 **지문 2개**(업로드키 `6D:90:DA:…` + Play앱서명키 `CE:DF:45:44:…`) 정상 배포됨. ⚠️ PWABuilder zip 안의 assetlinks.json 으로 **덮어쓰지 말 것** — 거기엔 업로드키 1개뿐이라 덮으면 URL 주소창이 다시 노출된다.
- [x] ✅ 앱 아이콘·스플래시·스토어 스크린샷·설명문 준비 완료 (아이콘·그래픽배너·폰스크린샷 6장·한글 설명 등록됨)
- [x] ✅ **스토어 등록·심사 제출 → 정식 게시(LIVE)** (약 2026-08-16)
- [x] ✅ **스토어 검색 최적화**(2026-08-20) — 앱이름 `CREAITE 크리에잇 - AI 시네마 OTT` + 간단한/자세한 설명에 한글 "크리에잇" 반영(제출→검토 중). 한글 "크리에잇" 검색 노출 개선용.
...
### 📊 방문자 분석 (2026-08-20 추가 — "1일 방문자"를 볼 수단이 없던 문제)

> 코드 배선 완료([analytics.ts](../src/app/utils/analytics.ts)·[main.tsx](../src/main.tsx)) + 개인정보처리방침 개정 완료(§5·§6·§8, 개정일 2026-08-20).
...
- [ ] ⚪ **카트 일괄 단일결제(2026-07-11)** — 현재 카트는 항목별 '구매하기'(단건 흐름 재사용)+합계 표시까지 구현. 진짜 "N편 한 번에 1결제"는 멀티아이템 주문(orders 라인아이템)+combined Toss 결제+confirm_payment 다건 라이선스 부여 = 백엔드 신규. **결제 OFF(payments_enabled=0)라 검증 불가 → live 결제 개통 후 작업.**
...
- [ ] 🟡 **시청 보호 잔여(2026-07-11 구매/시청 감사)** — Embed 토큰인증은 ON이라 임베드 플레이어는 프리미엄·연령 보호됨. **①연령게이트 — ✅ 발효 완료**(play-token 엔드포인트가 19금 미인증/비소유자에게 토큰 미발급. 코드 b34ce42(2026-07-11) → Edge 재배포 a50a679(2026-07-18)로 반영 확인. 2026-07-22 재확인 시 "재배포 필요"로 남아 있던 표기를 정정). **②잔여: CDN 토큰인증 OFF**라 `video_url`(anon 노출)·직접 `{id}/playlist.m3u8`·`play_720p.mp4` URL은 토큰 없이 접근 가능 → 비구독자/미성년이 직링크로 본편 시청 가능. CDN token auth ON 시 완전차단되나 preview.webp·썸네일 무토큰 서빙과 트레이드오프 → **출시 전 결정**(19금 실콘텐츠 생기기 전). 관련: [[upload-moderation-pipeline]] 잔여. **★2026-07-22 실측으로 정정** — 앞서 "videoId 만 알면 받을 수 있다"고 적었으나 **과장이었다.** 패널에 `Block direct url file access: ON` 이 켜져 있어 실제로 측정하면: **리퍼러 없음 → 403 / 리퍼러 `https://www.creaite.net/` 위조 → 200.** 즉 URL 을 그냥 붙여넣는 캐주얼 접근은 막히지만 **`curl -H "Referer: ..."` 한 줄이면 뚫린다**(리퍼러는 위조 자유). 유료 라이선스 보호로는 부족하므로 CDN token auth 는 여전히 필요하나, **"완전 무방비"는 아니다.** ▣ 같은 실측에서 나온 부작용: 우리 **다운로드 버튼도 리퍼러를 안 보내면 403** 이다 — 구코드는 `noopener,noreferrer` 로 창을 열어 리퍼러가 아예 안 갔다(주문 0건이라 미발현). CDN 토큰 서명을 붙이면 서명 URL 은 리퍼러와 무관하게 통과하므로 이 문제도 같이 해소된다. ▣ 패널 확인(2026-07-22): **CDN token auth 를 켜도 임베드 플레이어는 Bunny 가 자동 서명**한다고 명시돼 있어 상세페이지 재생은 안 깨진다. **Embed/CDN 토큰 키는 하나**(Token authentication key)라 새 시크릿 불필요. Allowed domains 에 localhost 3개 잔존 → 정리 대상. ▣ **2026-07-22 CDN 토큰 착수 결과 — 결론: 결제 개통 시 '판매 전용 마스터 분리' 로 처리.** ①서명 형식은 라이브 실측으로 **확정**(Basic MD5 + 디렉터리 토큰, 키는 임베드와 공용) → 메모리 [[bunny-cdn-token-format]]. Edge `/video-cdn-token` 발급·클라 유틸(`src/app/utils/cdnToken.ts`)·판정 스크립트(`scripts/bunny-cdn-verify.mjs`)까지 구현 완료. **토큰이 없으면 원본 URL 로 폴백하므로 배포돼 있어도 무영향**(Bunny 토글을 켜는 순간부터 효력). ②**'mp4 만 토큰' Edge Rule 안은 불가** — Stream 이 만든 풀존이 CDN 목록에 노출되지 않아(No pull zones found) Edge Rule 을 걸 수 없다. ③**전면 배선안도 목표 미달** — HLS 는 세그먼트 때문에 디렉터리 토큰이 필요한데, 그 토큰이 같은 폴더의 `play_*.mp4` 까지 커버한다(실측). ⇒ 스트리밍 가능한 사람 = mp4 취득 가능한 사람이라 라이선스 보호가 성립하지 않는다. ④따라서 **다운로드용 마스터를 재생용과 다른 존에 두고 그 존만 토큰 보호**하는 것이 정답. 구매본 인트로 카드('CREAITE 라이선스 제품' 표기)도 어차피 별도 마스터가 필요하므로 함께 처리한다. 주문 0건이라 지금 새는 돈은 없음. **★2026-07-22 최종 확정(사용자): 판매존 분리 방식으로 진행, 시점은 결제 개통 시.** 지금은 주문 0건이라 검증 불가 + 인트로 카드와 한 번에 처리(저장·인코딩 낭비 없음). 결제 개통 시 할 일: ①다운로드 전용 마스터를 재생용과 다른 Bunny 존/라이브러리에 생성 ②그 존만 CDN token auth ON ③마스터에 인트로 카드 합성("이 영상은 CREAITE에서 정당한 가격을 지불하고 All-in-One 라이선스를 획득한 제품입니다" — 영화라 통째 재배포되므로 손해 없고 정품 증명이 됨). 이미 구현된 자산(Edge /video-cdn-token·cdnToken.ts·bunny-cdn-verify.mjs·서명형식 [[bunny-cdn-token-format]])은 그대로 재사용.
