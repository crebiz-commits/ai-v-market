...
### ❌ KOE205 에러 (잘못된 요청)
**증상**: 카카오 로그인 시 "잘못된 요청 (KOE205)" 에러 메시지가 표시됨
**원인**: 앱에서 `account_email`(이메일) 권한을 요청하고 있으나, 카카오 개발자 센터에서 해당 항목이 활성화되지 않았기 때문입니다.

**해결 방법**:
1. [카카오 개발자 센터](https://developers.kakao.com/) 접속 -> 내 애플리케이션 선택
2. **카카오 로그인** -> **동의항목** 메뉴로 이동
3. **카카오계정(이메일)** 항목의 [설정] 버튼 클릭
4. **동의 목적**에 "사용자 식별 및 서비스 이용 안내" 등 간단한 사유 입력
5. **사용 설정**에서 `선택 동의` 또는 `필수 동의`로 변경 후 [저장]
   - *참고: '필수 동의'는 비즈니스 앱으로 전환해야 가능한 경우가 있으므로, 우선 '선택 동의'로 설정하셔도 됩니다.*
6. 저장 후 다시 로그인을 시도해 보세요.
